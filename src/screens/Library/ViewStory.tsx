...
const ViewStory = ({navigation}:Props) => {
  ...
  const handleSaveToLib = async () => {
    try {
      ...
      const res = await saveToLib({ storyId: allInfo?.id, token }).unwrap();
      if (res.code == 200) {
      ...
      <ScrollView>
        ...
        <View className='p-3'>
          ...
          <View className='flex-row gap-2 mt-2 mb-2'>
            <TouchableOpacity className='bg-[#D1FAE5] flex-1 rounded-lg p-4 items-center flex-row justify-center gap-2' onPress={handleSaveToLib}>
              <Entypo name="edit" size={16} color="#047857" />
              <Text className='text-[#047857] font-interMedium'>Save Story</Text>
            </TouchableOpacity>
